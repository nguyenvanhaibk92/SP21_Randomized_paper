...
%     C_INV = Regularization * eye(SIZE_A(2));
    ...
    RHS = A' * (SIGMA_INV(1, 1) * y_obs(:)) + C_INV * u0;  % Construct right hand side
    x_0 = zeros(size(RHS));
...
%     RHS = A' * (SIGMA_INV(1, 1) * y_obs(:)) + C_INV * u0; % Construct right hand side
%     x_0 = zeros(size(RHS));
%     matvecc = @(x) A' * (SIGMA_INV * (A * x)) + C_INV * x;
%     max_iters = 1000; tol = 1e-8;
%     u1 = CG(matvecc, RHS, x_0, max_iters, tol, true);
    
    norm(true_solution - u1) / norm(true_solution)
...
%                 EPSILON = normrnd(0, sqrt(1 / Regularization), [SIZE_A(2), r]);
                EPSILON = mvnrnd(zeros(SIZE_A(2),1),C, r)';
                ...
                % (1) using CG for solving Y = (SIGMA + A C A')^{-1} (d - A*u0)
                % (2) Then: u_RS  = C A' Y
                % (1) -----------------------------------------------------
                RHS = y_obs(:) - A*u0; % Equation 11
                x_0 = zeros(size(RHS));
            ...
            Error_Right(i, iter) = norm(result_RIGHT_in_ITER - u1) / norm(u1);
...
save ('Result','u1', 'result_LEFT','result_RIGHT', 'result_RAN_MAP', 'Error_LEFT', 'Error_Right', 'Error_RAN_MAP');
